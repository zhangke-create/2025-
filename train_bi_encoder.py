...
model_name = sys.argv[1] if len(sys.argv) > 1 else "/ai/ld/remote/rag/code/sentence-transformer/output/bi-encoder/stsb_augsbert_BM25_-ai-ld-pretrain-bge-base-zh--2024-01-22_12-57-19" #"bert-base-uncased"
...
cross_encoder_model_name = "/ai/ld/pretrain/bge-base-zh"
bi_encoder_path = (
...
#cross_encoder = CrossEncoder(cross_encoder_model_name, num_labels=1,device="cuda")
...
dev_samples = []
test_samples = []
my_path = "/ai/ld/remote/rag/code/sentence-transformer/training_data_sample.tsv.gz"
with gzip.open(my_path, "rt", encoding="utf8") as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
        score = float(row["score"])  # Normalize score to range 0 ... 1

        if row["split"] == "dev":
            dev_samples.append(InputExample(texts=[row["query"], row["item"]], label=score))
        elif row["split"] == "test":
            test_samples.append(InputExample(texts=[row["query"], row["item"]], label=score))
        else:
            # As we want to get symmetric scores, i.e. CrossEncoder(A,B) = CrossEncoder(B,A), we pass both combinations to the train set
            gold_samples.append(InputExample(texts=[row["query"], row["item"]], label=score))
            gold_samples.append(InputExample(texts=[row["item"], row["query"]], label=score))
